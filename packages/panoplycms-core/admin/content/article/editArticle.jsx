...
/*=======================================================
	this component is used display edit article
	where user can edit article 
========================================================*/
		...
		submitData(event){
			...
			// if(this.state.valid.form()){
				...
				Meteor.call('editArticle', FlowRouter.getParam("_id"), articleData, (err,data) => {
					if(err){
						AlertMessage('ERROR', i18n('ADMIN_CONTENTS_ARTICLES_ERROR_MESSAGE'), 'error');
					}else{
						AlertMessage('SUCCESS', i18n('ADMIN_CONTENTS_ARTICLES_SUCCESS_MESSAGE'), 'success');
					}
				});
			// }
		}
		render(){
			...
			let url=[{
		      ...
		      active:false
		    	...
		    	title:i18n('ADMIN_CONTENTS_ARTICLES_EDITARTICLE'),
			...
			return (
				<div>
					<Heading key={this.props.pageLoading}  data={i18n('ADMIN_CONTENTS_ARTICLES_EDITARTICLE')} url={url} />
	            ...
	            <div className="form-group">
						...
						<div className="panel-body custom-panel">
							...
							<div className="form-group">
								<label htmlFor="title" className="col-sm-2 control-label">{i18n('ADMIN_CONTENTS_ARTICLES_ADDARTICLE_FORM_TITLE')}</label>
								<div className = "col-sm-10">
									<input type = "text" id="title" name="title" className = "form-control" placeholder = {i18n('ADMIN_CONTENTS_ARTICLES_PLACEHOLDER_EDIT_TITLE')} defaultValue={this.props.results && this.props.results.title ? this.props.results.title:''} required />
								</div>
							</div>
							<div className="form-group">
								<label htmlFor="selectCategory" className="col-sm-2 control-label">{i18n('ADMIN_CONTENTS_ARTICLES_ADDARTICLE_FORM_CATEGORY')}</label>
								<div className="col-sm-10">
									<select selected="select" defaultValue={this.props.results && this.props.results.category ? this.props.results.category:''} name='SelectName' className="selectpicker form-control" id='selectCategory' data-style="btn-primary" required>
							...
							<div className="form-group">
								<label htmlFor="tokenfield" className="col-sm-2 control-label">{i18n('ADMIN_CONTENTS_ARTICLES_ADDARTICLE_FORM_TAGS')}</label>
								<div className="col-sm-10" id="token"> 
									<input type="text" id="tokenfield" className="form-control" defaultValue={this.props.results?this.state.tagTitle:''} required/>
								</div>
							</div>
							<div className="form-group">
								<label htmlFor="article" className="col-sm-2 control-label">{i18n('ADMIN_CONTENTS_ARTICLES_ADDARTICLE_FORM_ARTICLE')}</label>
								<div className="col-sm-10">
							...
							<div className="form-group">
								<label htmlFor="metaKeyword" className="col-sm-2 control-label">{i18n('ADMIN_CONTENTS_ARTICLES_ADDARTICLE_FORM_METAKEYWORD')}</label>
								<div className="col-sm-10">
							...
							<div className="form-group">
								<label htmlFor="metaDescription" className="col-sm-2 control-label">{i18n('ADMIN_CONTENTS_ARTICLES_ADDARTICLE_FORM_METADESCRIPTION')}</label>
								<div className="col-sm-10">
