...
Meteor.methods({
    ...
    getTag: function(tagId){
        return tags.find({ _id: tagId, status: 1 },{fields:{tag:1, alias:1}}).fetch();
    },
    getAddedTagList: function(){
        return tags.find({ status: 1 },{fields:{tag:1, alias:1}}).fetch();
    },
    ...
    update_menu: function(id,categoryname,aliasname,type,id_of_menu_item_sub_type,id_of_parent){
         var obj1={
            $set: {
            	title:categoryname,
            	alias:aliasname,
    ...
    add_htmlblocks: function(title,htmlblock,position,menuname){
        htmlblocks.insert({
            title: title,
            html: htmlblock,
            position:position,
