    ...
    Meteor.methods({
        ...
        deleteMenus:function(id){
            return (
                ...
                PanoplyCMSCollections.MenuItems.update({mainParentId:id},{$set:{"trash":true}},{multi:true})
            )
        },
        deleteMenuParmanent:function(id){
            PanoplyCMSCollections.Menus.remove({_id:id});
            PanoplyCMSCollections.MenuItems.remove({mainParentId:id})
        },
        restoreMenus:function(id){
            return (
                PanoplyCMSCollections.Menus.update(id,{$set:{"trash":false}}),
                PanoplyCMSCollections.MenuItems.update({mainParentId:id},{$set:{"trash":false}},{multi:true})
            )
        },
