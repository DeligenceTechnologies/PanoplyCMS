...
/*=======================================================
	this component is used display list of categories
========================================================*/
...
import('react').then(({Component})=>{
	...
	class ListCategories extends Component {
		...
		render() {
			...
			if(this.props.Categories.length == 0  && this.state.trashListShow == false){
				nodata = <NotFoundComp />
			}else if(this.props.Categories.length == 0 && this.state.trashListShow == true){
				nodata = <NotFoundComp />
			}else{
				nodata = '';
			}
			...
			return (
				<div>
					<Heading data={i18n('ADMIN_CONTENTS_CATEGORY_CATEGORY')} url={url}/>
					<div className="custom-table">
		              ...
		              <i className="fa fa-plus-circle fa-lg"></i>&nbsp;
			            {i18n('ADMIN_CONTENTS_CATEGORY_ADDCATEGORY')}
	            	</a>
								...
								<div className="table-responsive">
									{
										nodata == '' ?
											<table className="table table-striped table-bordered table-list table-hover">
												...
												<thead>
													<tr>
														<th>{i18n('ADMIN_CONTENTS_CATEGORY_ADDCATEGORY_FORM_CATEGORYNAME')}</th>
														<th>Action</th>
													</tr>
												</thead>
												<tbody>
	...
	import('meteor/react-meteor-data').then(({createContainer})=>{
		export default createContainer((props)=>{
			...
			return{
				pageLoading: !ready,
				Categories: PanoplyCMSCollections.Categories.find({trash:Session.get('trash')},{limit: props.dict.get('limit')}).fetch(),
				categoriesCount: PanoplyCMSCollections.Categories.find({trash:Session.get('trash')}).count(),
